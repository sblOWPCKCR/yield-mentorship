  ...
  describe("GreenVault", function () {
    ...
    beforeEach(async function () {
      ...
      this.gvault = (<GreenVault>await deployContract(this.signers.admin,
        await hre.artifacts.readArtifact("GreenVault"), [])).connect(this.signers.user1);
    ...
    it("starts empty", async function () {
      expect(await this.gvault.deposits(this.signers.user1.address, this.gtoken.address))
        .to.be.equal(0);
    });

    it("doesn't accepts transfers without approvals", async function () {
      await expect(this.gvault.deposit(this.gtoken.address, starting_balance))
    ...
    it("doesn't accepts out-of-balance transfers", async function () {
      ...
      await expect(this.gvault.deposit(this.gtoken.address, starting_balance * 2))
        .to.be.reverted; // can't check for exact message since it's ERC20-generated and is not prescribed
    });
    ...
    describe("transfers", function () {
      ...
      it("don't change other user's balance", async function () {
        await this.gvault.deposit(this.gtoken.address, starting_balance);

        expect(await this.gvault
            .connect(this.signers.user2)
            .deposits(this.signers.user2.address, this.gtoken.address))
          .to.be.equal(0);
      ...
      it("don't change other token's balance", async function () {
        ...
        expect(await this.gvault.deposits(this.signers.user1.address, this.another_token.address))
          .to.be.equal(0);
    ...
    describe("withdrawals", function () {
      ...
      it("by different users are not permitted", async function () {
        await expect(this.gvault
            .connect(this.signers.user2)
            .withdraw(this.gtoken.address, 1))
          .to.be.revertedWith("InsufficientBalance");
      })

      it("to different token are not permitted", async function () {
        await expect(this.gvault.withdraw(this.another_token.address, 1))
          .to.be.revertedWith("InsufficientBalance");
      })

      it("with insufficient balance are not permitted ", async function () {
        await expect(this.gvault.withdraw(this.gtoken.address, starting_balance * 2))
          .to.be.revertedWith("InsufficientBalance");
      })

      it("change the balance", async function () {
        await expect(() => this.gvault.withdraw(this.gtoken.address, starting_balance))
          .to.changeTokenBalances(this.gtoken, [this.signers.user1, this.gvault], [starting_balance, -starting_balance]);
